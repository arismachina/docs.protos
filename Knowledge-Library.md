...
- **Tables in spreadsheets** are read as tables, not as flattened text. That lets the Co-Engineer pull a specific column and filter rows on a value. It returns the rows themselves: there is no summing or averaging a column. Where a sheet is too ambiguous to read confidently, Protos says so on the document instead of guessing at it.
